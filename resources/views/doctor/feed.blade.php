    ...
    <div class="card-body">
      <div class="d-flex justify-content-between">
        <div class="fw-semibold">
          {{ $post->is_anonymous ? 'Anonymous Doctor' : ($post->doctor->name ?? 'Doctor') }}
        </div>
      ...
      <div class="mt-2">
        ...
        @foreach($post->comments as $c)
          <div class="p-2 border rounded mb-2">
            <div class="small text-muted">{{ $c->doctor->name ?? 'Doctor' }} — {{ $c->created_at->diffForHumans() }}</div>
            <div>{{ $c->content }}</div>
          </div>
        @endforeach
...
@empty
  <div class="card"><div class="card-body text-muted">No posts yet.</div></div>
@endforelse
